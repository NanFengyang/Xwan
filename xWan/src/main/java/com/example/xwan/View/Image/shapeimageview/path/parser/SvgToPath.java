package com.example.xwan.View.Image.shapeimageview.path.parser;
...
public class SvgToPath {
	...
	private void pushTransform(XmlPullParser atts) {
		...
		final Matrix matrix = transform == null ? IDENTITY_MATRIX
				: TransformParser.parseTransform(transform);
	...
	void startElement() {
		...
		if (localName.equals("svg")) {
			...
			NumberParse viewbox = NumberParse.getNumberParseAttr("viewBox",
					atts);

			pushPath();
			Matrix matrix = IDENTITY_MATRIX;

			if (viewbox != null && viewbox.numbers != null
					&& viewbox.numbers.size() == 4) {
				if (width < 0.1f || height < -0.1f) {
					...
					float sx = width
							/ (viewbox.numbers.get(2) - viewbox.numbers.get(0));
					float sy = height
							/ (viewbox.numbers.get(3) - viewbox.numbers.get(1));
			...
			NumberParse numbers = NumberParse
					.getNumberParseAttr("points", atts);
			if (numbers != null) {
				Path p = new Path();
			...
			Log.d(TAG, String.format("Unrecognized tag: %s (%s)", localName,
					showAttributes(atts)));
		}
	}
	...
	final Float getFloatAttr(String name, XmlPullParser attributes,
			Float defaultValue) {
		Float result = ParseUtil.convertUnits(name, attributes, dpi, width,
				height);
