package com.example.xwan.View.flipviewpager;
...
/**
 * onInterceptTouchEvent() modified by Tom-Philipp Seifert to allow delegation
 * of click events to buttons etc.
 * 
 * @author Yalantis
 */
public class FlipViewPager extends FrameLayout {
	...
	private final HashMap<Integer, PageItem> pages = new HashMap<Integer, PageItem>();
	...
	public void setOnChangePageListener(
	...
	private void drawFlippingHalf(Canvas canvas) {
		...
		mCamera.rotateY(getDegreesDone() > 90 ? 180 - getDegreesDone()
				: -getDegreesDone());
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			this.mActivePointerId = ev.getAction()
	...
	private void checkIfChildWasClicked(MotionEvent ev, final View childView) {
		if (childView.isClickable()
				&& isPointInsideView(ev.getRawX(), ev.getRawY(), childView)) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (flipping) {
				...
				float deltaFlipDistance = mLastMotionX
				...
				boolean isOverFlipping = mFlipDistance < minFlipDistance
						|| mFlipDistance > maxFlipDistance;

				if (isOverFlipping) {
					this.overFlipping = true;
					toggleFlip(flipping);
					setFlipDistance(calculate(mFlipDistance, minFlipDistance,
							maxFlipDistance));
		...
		case MotionEvent.ACTION_CANCEL:
			if (flipping) {
				...
				flipToPage(getNextPage((int) mVelocityTracker
	...
	private void drawFlippingShadeShine(Canvas canvas) {
		if (getDegreesDone() < 90) {
			mShinePaint
					.setAlpha((int) ((getDegreesDone() / 90f) * FLIP_SHADE_ALPHA));
			canvas.drawRect(mRightRect, mShinePaint);
		} else {
			mShadePaint
					.setAlpha((int) ((Math.abs(getDegreesDone() - 180) / 90f) * FLIP_SHADE_ALPHA));
	...
	public boolean drawEdges(Canvas canvas) {
		...
		if (rightNotFinished || !mLeftEdgeEffect.isFinished()) {
			...
			canvas.translate(rightNotFinished ? -getHeight() : 0,
					rightNotFinished ? 0 : -getWidth());
			shouldContinue = rightNotFinished ? mRightEdgeEffect.draw(canvas)
					: mLeftEdgeEffect.draw(canvas);
			canvas.restore();
		}
	...
	public float calculate(float flipDistance, float minFlipDistance,
		...
		float deltaOverFlip = flipDistance
				- (flipDistance < 0 ? minFlipDistance : maxFlipDistance);
		if (deltaOverFlip > 0) {
	...
	public void setAdapter(ListAdapter adapter, int activePage, int row,
			int maxItems) {
		this.adapter = adapter;
		removeAllViews();
		// For case we're showing row with less items than we storing
		if (pages.size() > adapter.getCount())
			pages.clear();
		for (int i = 0; i < adapter.getCount(); i++) {
			PageItem item = pages.containsKey(i) ? pages.get(i)
					: new PageItem();
			item.pageView = adapter.getView(i,
					pages.containsKey(i) ? pages.get(i).pageView : null, this);
			pages.put(i, item);
		}
		...
		mFlipDistance = -1;
		setFlipDistance(0);
		mScroller.startScroll(0, (int) mFlipDistance, 0, (int) (activePage
				* FLIP_DISTANCE - mFlipDistance), getFlipDuration(0));
	}

	public void flipToPage(int page) {
		int delta = page * FLIP_DISTANCE - (int) mFlipDistance;
		endFlip();
		mScroller.startScroll(0, (int) mFlipDistance, 0, delta,
				getFlipDuration(delta));
		invalidate();
	}
}
